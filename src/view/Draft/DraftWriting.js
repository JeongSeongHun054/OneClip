...
import { draftWritingRemove } from "../../modules/draftModules";
      ...
      <Ul>
        {data.slice(offset, offset + PER_PAGE).map((res) => (
          <li key={res.id}>
            ...
            <button onClick={() => onRemove(res.id)}>회수</button>
            <Link to="#">
...
const DraftWriting = () => {
  const data = useSelector((state) => state.draftModules.datas);
  const dispatch = useDispatch();
  const onRemove = useCallback((id) => dispatch(draftWritingRemove(id)), [
    dispatch,
  ]);
  ...
  const handleWriting = () => {
    history.push("/draft/draftWriting/writing");
  };
  ...
  return (
    <Wrapper>
      <Content>
        <ContentBody>
          <PageTitle>기안함</PageTitle>
          <Button onClick={handleWriting}>기안 작성</Button>
